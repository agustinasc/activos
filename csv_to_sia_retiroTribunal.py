...
COLS_DEFAULT_CERO = {
    ...
    'aporte_dif', 'remu_doc', 'cant_hsext', 'remusac', 'asigfliar', 'tras_vol',
}
...
COLS_MONETARIO = {
    ...
    'remu_doc', 'ap_dcto788', 'aporte_dif',
}
...
def formatear_valor(valor, ancho, alineacion, campo=''):
    ...
    if es_vacio and campo in COLS_DEFAULT_CERO:
        ...
        # Convertir floats que pandas genera para enteros grandes:
        # '1.11800000000' (sin e) -> quitar el punto -> '111800000000'
        # '1.118e+11'             -> int(float)      -> '111800000000'
        if s and '.' in s:
            partes = s.split('.')
            dec_digits = len(partes[1]) if len(partes) == 2 else 0
            no_dec = s.replace('.', '')
            if no_dec.isdigit() and len(no_dec) == ancho and dec_digits > 2:
                # Caso '1.11800000000' con ancho=12 -> '111800000000'
                # (solo si dec_digits>2, para no confundir con valores monetarios)
                s = no_dec
            elif ('e+' in s.lower() or 'e-' in s.lower() or
                    (no_dec.isdigit() and dec_digits == 1 and len(no_dec) > 6)):
                # Caso '111800000000.0' o notación científica -> int
                try:
                    s = str(int(float(s)))
...
def main(csv_path, output_path=None):
    ...
    for idx, fila in df.iterrows():
        ...
        # Retiro: valores ya pre-calculados en el CSV.
        # El periodo se usa tal cual (formato AAAAMM, no se convierte).
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Uso: python csv_to_sia_retiro.py <archivo.csv> [salida.txt]")
        sys.exit(1)
    csv_path = sys.argv[1]
