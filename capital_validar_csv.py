...
# ANSES RS 74/2026
ANSES_BASE_MIN    = 128091.45
ANSES_BASE_MAX    = 4162912.57
ANSES_APORTE_MIN  = 14090.06
ANSES_APORTE_MAX  = 457920.38
ANSES_CONTRIB_MIN = 13026.90

# Caja Complementaria (Municipios / Poder Ejecutivo / Poder Judicial)
CAJA_BASE_MIN    = 357800.00
CAJA_APORTE_MIN  = 10734.00
CAJA_APORTE_MAX  = 209463.63   # Municipios/Ejecutivo (Judicial: 374789.32)
CAJA_PCT_APORTE  = 0.03
CAJA_PCT_CONTRIB = 0.03

# Obra Social OSEP Dcto 142/25
OSEP_APORTE_MIN  = 33846.96    # apafilos mínimo
OSEP_CONTRIB_MIN = 67693.91    # appatros mínimo
OSEP_TRASP_MIN   = 3760.77     # trasp_osep mínimo
OSEP_PCT_APORTE  = 0.045
OSEP_PCT_CONTRIB = 0.09
OSEP_PCT_TRASP   = 0.005       # 0.5% de remuimpoos
OSEP_BASE_MIN    = 752154.63
APO_VOL_MIN_1    = round(OSEP_BASE_MIN * 0.025, 2)   # $18.803,87 (1 adherente)
APO_VOL_MIN_2    = round(OSEP_BASE_MIN * 0.05,  2)   # $37.607,73 (2+ adherentes)
TRAS_VOL_MIN     = round(OSEP_BASE_MIN * 0.005, 2)   # $3.760,77
...
def es_num(s):
    # Acepta cualquier cantidad de dígitos (incluyendo ceros iniciales) con hasta 2 decimales
    return bool(re.match(r'^[0-9]+(\.[0-9]{1,2})?$', s.strip())) if s.strip() else False
