...
COLS_INT_VALUE    = {'porceadi', 'adherente', 'codigo', 'por_retiro'}
COLS_DECIMAL      = {'dias_traba', 'cos_alt', 'cos_int', 'apafilos', 'appatros', 'aportejubi'}
COLS_DEFAULT_CERO = {'ap_dcto788'}
COLS_NUMERICO_ANCHO = {'remutotal', 'remuimpo', 'remuimpoos'}

# Campos numéricos con .2f que en el CSV vienen con ceros a la izquierda (ej: '000000.00')
# Se normalizan a float para eliminar esos ceros antes de formatear
COLS_NUMERICO_NORM = {
    'aportvolun', 'exessocial', 'asigfliar', 'exeosocial', 'impoadicos',
    'coseguro', 'd_svco', 'd_ss', 'd_svca', 'd_sc', 'remusac', 'hsextras',
    'ap_dcto788', 'aporte_caj', 'contrib_ca', 'deuda_caja', 'trasp_osep',
    'premios', 'vacaciones', 'remuimpoto', 'rem_retiro', 'apo_vol', 'tras_vol',
    'cos_pro', 'canasta_fl', 'conceptono', 'aporte_dif', 'aportepatr',
    'nroagente', 'ubicacion', 'cant_hsext', 'canthijo', 'rebpromo', 'porcreduc',
    'remusueldo', 'remu_doc', 'apafilos', 'appatros', 'aportejubi',
}
...
def format_num(v, ancho):
    """Formatea un número para que entre en `ancho` chars.
    Intenta primero 2 decimales, luego 1 (con ROUND, no trunc), luego entero."""
    s2 = f"{round(v, 2):.2f}"
    if len(s2) <= ancho:
        return s2
    s1 = f"{round(v, 1):.1f}"
...
def recalcular(fila):
    """Recalcula remuimpo, remutotal y remuimpoos coherentemente."""
    rs   = fv(fila, 'remusueldo')
    rc   = fv(fila, 'remusac')
    hx   = fv(fila, 'hsextras')
    prem = fv(fila, 'premios')
    vac  = fv(fila, 'vacaciones')
    can  = fv(fila, 'canasta_fl')
    con  = fv(fila, 'conceptono')

    ri = round(rs + rc + hx, 2)
    rt = round(ri + prem + vac + can + con, 2)

    fila = fila.copy()
    fila['remuimpo']   = format_num(ri, 9)
    fila['remutotal']  = format_num(rt, 9)
    fila['remuimpoos'] = format_num(rt, 9)
    return fila
...
def formatear_valor(valor, ancho, alineacion, campo=''):
    ...
    if campo in COLS_NUMERICO_ANCHO and s:
        try:
            s = format_num(float(s), ancho)
        except ValueError:
            pass

    elif campo in COLS_NUMERICO_NORM and s:
        # Normaliza ceros a la izquierda: '000000.00' -> '0.00', '00870389' -> '870389'
        try:
            v = float(s)
            # Si el valor original no tiene punto decimal, formatearlo como entero
            if '.' in s:
                s = f"{v:.2f}"
            else:
                s = str(int(v))
...
def main(csv_path, output_path=None):
    ...
    if incorrectas:
        ...
        print(f"Longitud         : 555 bytes por linea OK")
    ...
    if errores_cuil:
        ...
        print(f"CUILes           : todos validos OK")
